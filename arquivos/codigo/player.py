...
class Player(pygame.sprite.Sprite):
	def __init__(self, posicao_inicial, caminho, game_manager, group_do_laser, group_do_meteoro, groups):
		...
		# sons
		self.som_do_laser = pygame.mixer.Sound('../sons/som_do_laser.aiff')
	...
	def input(self):
		...
		if keys[pygame.K_SPACE] and self.pode_atirar:
			laser = Laser(self.rect.midtop, CAMINHOS['Laser'], self.game_manager, self.group_do_meteoro, self.group_do_laser)
	...
	def limitar_movimento(self):
		...
		# limita a movimentação para a esquerda
		if self.rect.left <= 10:
			self.posicao.x = 10 + self.rect.width * 0.5
			self.rect.left = 10

		# limita a movimentação para a direita
		...
		# limita a movimentação para cima
		if self.rect.top <= 10:
			self.posicao.y = 10 + self.rect.height * 0.5
			self.rect.top = 10

		# limita a movimentação para baixo
